...
const debugPricingVsRateSlabs = async () => {
  try {
    ...
    for (let i = 0; i < allProducts.length; i++) {
      ...
      if (product.rateSlabs && product.rateSlabs.length > 0) {
        ...
        product.rateSlabs.forEach((slab, index) => {
          console.log(`      ${index + 1}. Min: ${slab.minQuantity}, Max: ${slab.maxQuantity}, Rate: ₹${slab.rate}`);
        });
      } else {
      ...
      if (availablePrice) {
        ...
        console.log(`   ✅ WOULD BE INCLUDED in bulk pricing`);
      } else {
    ...
    // Get products that would be found by current backend logic
    ...
    pricingRecords.forEach((pricing, index) => {
      console.log(`   ${index + 1}. ${pricing.product.itemName} - ₹${pricing.sellingPrice}`);
    });
    ...
    if (allProducts.length > pricingRecords.length) {
      console.log('\n❌ PROBLEM: Some products are excluded because they only have rate slabs but no DealerPricing records');
      console.log('💡 SOLUTION: Include products with rate slabs even if they don\'t have DealerPricing records');
    }
