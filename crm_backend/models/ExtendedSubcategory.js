...
extendedSubcategorySchema.methods.getFullPath = async function() {
  ...
  while (current.parentExtendedSubcategory) {
    current = await mongoose.model('ExtendedSubcategory').findById(current.parentExtendedSubcategory);
    if (current) {
...
extendedSubcategorySchema.statics.getChildren = function(parentId, categoryId, subcategoryId) {
  return this.find({
...
extendedSubcategorySchema.statics.getByLevel = function(level, categoryId, subcategoryId, parentId = null) {
  const query = {
    level: level,
    category: categoryId,
    subcategory: subcategoryId,
