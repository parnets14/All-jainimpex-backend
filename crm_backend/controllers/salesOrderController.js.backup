...
export const createSalesOrder = async (req, res) => {
  try {
    ...
    if (dealerData.creditLimit && dealerData.creditLimit > 0) {
      ...
      console.log(`Ã°Å¸â€™Â³ Credit Limit Check:`, {
      ...
      if (newOutstanding > dealerData.creditLimit) {
        ...
        console.log(`Ã¢Å¡Â Ã¯Â¸Â Credit limit exceeded by Ã¢â€šÂ¹${overlimitAmount.toFixed(2)}`);
    ...
    if (isOutOfStock) {
      ...
      console.log("Ã°Å¸Å¡Â¨ Creating out-of-stock sales order - status locked to Pending");
    }
    ...
    if (salesOrder.status === "Pending") {
      ...
      console.log(`Ã°Å¸â€œâ€¦ Automatic expiry set for pending order ${orderNumber}: ${expiryDate.toISOString()}`);
    }
    ...
    if (!isOutOfStock) {
      ...
      console.log("Ã°Å¸Å¡Â¨ Out-of-stock order created - no stock movements will be made until stock is available");
    }
    ...
    if (error.name === 'ValidationError') {
      ...
      console.error('Ã¢ÂÅ’ Validation Error Details:', details);
      return res.status(400).json({
        success: false,
...
export const updateSalesOrderStatus = async (req, res) => {
  try {
    ...
    if (salesOrder.isOutOfStock && warehouseAssigned) {
      console.log("Ã°Å¸Å½Â¯ Warehouse assigned to out-of-stock order - clearing isOutOfStock flag");
    ...
    if (salesOrder.creditOverlimit && 
      ...
      return res.status(400).json({
        success: false,
        message: `Cannot change status - Credit limit exceeded by Ã¢â€šÂ¹${(salesOrder.creditOverlimit.overlimitAmount || 0).toLocaleString()}. Super Admin approval required before this order can proceed.`
      });
    }
    ...
    if (!salesOrder.isOutOfStock) {
      ...
      console.log("Ã°Å¸Å¡Â¨ Out-of-stock order - no stock movements will be made");
    }
    ...
    if (originalStatus !== status) {
      try {
        ...
        console.log(`Ã°Å¸â€œÂ§ Notification created for dealer ${salesOrder.dealer}: ${message} (Status changed from ${originalStatus} to ${status})`);
...
export const updateSalesOrder = async (req, res) => {
  try {
    ...
    // Debug logging
    console.log("Ã°Å¸â€Â Update Sales Order - Status Check:");
    ...
    if (!currentStatus || !editableStatuses.includes(currentStatus)) {
      ...
      console.log("Ã¢ÂÅ’ Edit Rejected:", errorMessage);
      return res.status(400).json({
        success: false,
    ...
    console.log("Ã¢Å“â€¦ Edit Allowed for status:", currentStatus);
    ...
    if (newStatus && newStatus !== originalStatus) {
      try {
        ...
        console.log(`Ã°Å¸â€œÂ§ Notification created for dealer ${updatedOrder.dealer}: ${message} (Status changed from ${originalStatus} to ${newStatus})`);
      } catch (notificationError) {
        console.error('Error creating notification:', notificationError);
      }
    }
...
async function createSingleSalesOrder(orderData, userId) {
  ...
  if (dealerData.creditLimit && dealerData.creditLimit > 0) {
    ...
    console.log(`Ã°Å¸â€™Â³ Credit Limit Check (Single Order):`, {
      creditLimit: dealerData.creditLimit,
      currentOutstanding,
...
export const createSalesOrderWithAutoSplit = async (req, res) => {
  try {
    console.log("Ã°Å¸Å¡â‚¬ createSalesOrderWithAutoSplit called");
    ...
    console.log("Ã¢Å“â€¦ Dealer found:", dealerData.name);
    console.log("Ã°Å¸â€œÅ  Dealer credit days - Regular:", dealerData.creditDaysRegular, "CD:", dealerData.creditDaysCD);
    ...
    console.log("Ã°Å¸â€œÂ¦ Product details loaded:", productDetails.length, "products");
    ...
    console.log("Ã°Å¸â€Âµ Regular Sale products:", regularProducts.length);
    console.log("Ã°Å¸Å¸Â¢ CD Sales products:", cdProducts.length);
    
    const createdOrders = [];
    
    // Create Regular Sales Order if there are regular products
    if (regularProducts.length > 0) {
      console.log("\nÃ°Å¸â€œÂ Creating Regular Sales Order...");
      ...
      console.log("Ã¢Å“â€¦ Regular Sales Order created:", regularOrder.orderNumber);
    }
    
    // Create CD Sales Order if there are CD products
    if (cdProducts.length > 0) {
      console.log("\nÃ°Å¸â€œÂ Creating CD Sales Order...");
      const cdOrderData = {
        ...orderData,
        dealer,
        products: cdProducts,
      ...
      console.log("Ã¢Å“â€¦ CD Sales Order created:", cdOrder.orderNumber);
    }
    ...
    if (createdOrders.length === 0) {
      ...
      console.log("\nÃ¢Å“â€¦ Single order created successfully");
      res.status(201).json({
        success: true,
      ...
      console.log("\nÃ¢Å“â€¦ Orders split successfully - 2 orders created");
      res.status(201).json({
        success: true,
    ...
    console.error("Ã¢ÂÅ’ Create Sales Order with Auto-Split Error:", error);
